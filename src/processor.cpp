...
Processor::Processor(int tid) : tid_(tid), status_(PRO_STOPPED), pLoop_(nullptr), runningNewQue_(0), pCurCoroutine_(nullptr), mainCtx_(0)
{
    ...
    pLoop_ = new std::thread(
        [this]
        {
            ...
            while(PRO_RUNNING == status_)
            {
                ...
                epoller_.getActEvServ(epollTimeOutMs, actCoroutines_);
